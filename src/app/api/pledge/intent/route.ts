...
export async function POST(req: Request) {
  try {
    ...
    if (!campaign || !amount) return NextResponse.json({ ok:false, error:'Missing fields' }, { status: 400 })
    ...
    const { data: camp, error: cErr } = await db.from('campaigns').select('id,slug').eq('slug', campaign).single()
