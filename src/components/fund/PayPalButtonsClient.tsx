// src/components/fund/PayPalButtonsClient.tsx
'use client';
...
export default function PayPalButtonsClient(props: {
  amount: number;
  tierKey?: string;
  label: string;
  ...
  const { amount, tierKey, label, env, clientId, origin } = props;
  ...
  useEffect(() => {
    ...
    if (env === 'sandbox') params.set('buyer-country', 'US');
  ...
  useEffect(() => {
    ...
    const buttons = window.paypal.Buttons({
      ...
      createOrder: async (_: any, actions: any) => {
        setError(null);
        // (A) optional: tell our API about the pledge (intent)
        try {
      ...
      onApprove: async (_: any, actions: any) => {
        try {
          ...
          // notify server to finalize pledge
          try {
    ...
    else setError('PayPal is not available for your browser/region.');
